...
<body>
    <div id="top-bar">
        <a href="https://github.com/SIN0NIS/hots_talent_build_auto_git" target="_blank">🔗 GitHub Link</a>
    ...
    <script>
        ...
        function renderList(l) {{ document.getElementById("hero-list-dropdown").innerHTML = l.map(h => `<div class="hero-item" onclick="selectHero('${{h.id}}')">${{currentLang==='ko'?h.name_ko:h.name_en}}</div>`).join(""); }}
        ...
        function selectHero(id) {{
            ...
            let h = ''; lvs.forEach((lv, i) => {{
                ...
                h += `</div><div class="t-info-display" id="desc-${{i}}" style="font-size:0.9em; color:#aaa;">${{currentLang==='ko'?'특성을 선택하세요.':'Select a Talent.'}}</div></div>`;
            }});
        ...
        function refreshTalentUI() {{
            ...
            selectedTalents.forEach((tn, ti) => {{
                ...
                }} else {{ box.innerHTML = currentLang==='ko'?'특성을 선택하세요.':'Select a Talent.'; }}
            }});
            updateBuildUI();
        }}
        ...
        function updateBuildUI() {{
            ...
            let summaryHTML = "";
            selectedTalents.forEach((tn, ti) => {{
                if(tn > 0) summaryHTML += `<img src="${{imgBase}}${{hData.talents[lvs[ti]][tn-1].icon}}" class="summary-icon filled">`;
        ...
        function renderAbilities() {{
            ...
            const process = (list) => {{ if(!list) return; list.forEach(s => {{ html += `<div class="ability-item"><img src="${{imgBase}}${{s.icon}}" class="ability-icon"><div style="flex:1"><b style="color:var(--blue)">[${{s.abilityType}}] ${{s.name}}</b><div style="color:#bbb; font-size:0.95em;">${{processTooltip(s.fullTooltip||s.description, currentLevel)}}</div></div></div>`; }}); }};
        ...
        async function takeScreenshot(mode) {{
            ...
            try {{
                ...
                if (mode === 'save') {{
                    const link = document.createElement('a'); link.download = `${{getActiveData()[currentHeroId].name}}_build.png`; link.href = canvas.toDataURL(); link.click();
                }} else {{
