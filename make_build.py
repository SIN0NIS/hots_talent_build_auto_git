...
<head>
    ...
    <style>
        ...
        .stat-grid {{ display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; margin-bottom: 15px; }}
        ...
        .ability-list {{ border-top: 1px solid #333; padding-top: 10px; display: flex; flex-direction: column; gap: 8px; }}
        ...
        /* 모바일 가로 너비의 주범인 tier-row 수정 */
        .tier-row {{ display: flex; align-items: center; background: var(--card); padding: 6px 8px; border-radius: 6px; border-left: 4px solid var(--p); gap: 6px; margin: 3px 5px; box-sizing: border-box; width: auto; }}
        .tier-label {{ color: var(--blue); font-weight: bold; width: 32px; flex-shrink: 0; font-size: 11px; text-align: center; }}
        .t-icon {{ width: 36px; height: 36px; border: 1px solid #444; cursor: pointer; border-radius: 5px; background: #000; flex-shrink: 0; }}
        .t-icon.selected {{ border-color: var(--gold); box-shadow: 0 0 8px var(--gold); transform: scale(1.05); z-index: 10; }}
        
        /* 텍스트 영역이 밀려나지 않도록 수정 */
        .t-info-display {{ flex: 1; font-size: 10px; color: #ccc; line-height: 1.3; padding-left: 8px; border-left: 1px solid #444; min-height: 36px; overflow: hidden; display: flex; align-items: center; }}
...
<body>
    ...
    <div id="capture-area">
        <div id="welcome-area">
            ...
            <div style="font-size:16px; color:#555;">영웅을 선택하면 상세 정보가 나타납니다.</div>
        </div>
        <div id="hero-stat-container">
            <div id="hero-title-area" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <div id="hero-info-title" style="font-size:18px; font-weight:bold; color:var(--p);">영웅 이름</div>
                <div id="hero-role-badge" style="font-size:10px; color:var(--blue); border:1px solid var(--blue); padding:1px 4px; border-radius:4px;">역할군</div>
            </div>
            <div id="level-control" style="background:#25252b; padding:8px; border-radius:6px; margin-bottom:10px; display:flex; align-items:center; gap:10px;">
    ...
    <div id="footer">
        ...
        <div style="display:flex; gap:10px; width:98%;">
            <div id="build-code" onclick="copyCode()" style="flex:3; font-size:12px; color:var(--gold); background:#1a1a1a; padding:8px; border-radius:6px; border:1px dashed var(--gold); text-align:center;">[영웅을 선택하세요]</div>
            <button onclick="takeScreenshot()" style="flex:1; background:var(--p); color:white; border:none; border-radius:6px; font-weight:bold; font-size:13px;">📸 저장</button>
        </div>
    </div>
    <script>
        ...
        function selectHero(id, code = null) {{
            ...
            let h = ''; lvs.forEach((l, i) => {{
                ...
                h += `<div class="tier-row"><div class="tier-label">${{n}}L</div><div style="display:flex;gap:4px;">`;
        ...
        function toggleTalent(ti, tn, el) {{
            ...
            if(selectedTalents[ti] === tn) {{ selectedTalents[ti] = 0; el.classList.remove("selected"); box.innerHTML = "선택..."; }}
                ...
                box.innerHTML = `<b style="color:#fff;">${{t.name}}</b>`;
            }}
            updateUI();
        }}
        ...
        function renderStats() {{
            ...
            const sArr = [
                ...
                {{l:"공격", v:c(w.damage, w.damageScale, currentLevel)}}, 
                {{l:"거리", v:w.range.toFixed(1)}}
            ];
